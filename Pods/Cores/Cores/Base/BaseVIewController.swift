...
open class BaseVIewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func setNavigationBarWithTitleAndLikeButton(title: String, isFavorite: Bool) {
        setNavigationBarWithTitle(title: title)
        
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 22, height: 16))
        if isFavorite {
            button.setImage(UIImage(named: "ic_love_black", in: Bundle(identifier: CoreBundle.getIdentifier()), compatibleWith: nil), for: .normal)
        } else {
            button.setImage(UIImage(named: "ic_love_gray", in: Bundle(identifier: CoreBundle.getIdentifier()), compatibleWith: nil), for: .normal)
        }
    ...
    @objc open func onRequestFavorite() {
        
    }
    
    open func setNavigationBarWithTitleAndSaveButton(title: String) {
        setNavigationBarWithTitle(title: title)
